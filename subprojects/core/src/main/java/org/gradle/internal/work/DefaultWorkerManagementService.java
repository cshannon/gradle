...
public class DefaultWorkerManagementService implements WorkerManagementService {
    ...
    private final Multimap<Long, ProjectLock> threadProjectMap = Multimaps.synchronizedListMultimap(ArrayListMultimap.<Long, ProjectLock>create());
    ...
    private boolean tryLockProject(String projectPath) {
        ProjectLock projectLock = getProjectLock(projectPath);
        return projectLock.tryLock();
    }

    private ProjectLock lockProject(String projectPath) {
        ProjectLock projectLock = getProjectLock(projectPath);
        projectLock.lock();
        return projectLock;
    }

    private ProjectLock unlockProject(String projectPath) {
        ProjectLock projectLock = getProjectLock(projectPath);
        if (projectLock != null) {
            projectLock.unlock();
        }
    ...
    public boolean isProjectLocked(String projectPath) {
        ProjectLock projectLock = getProjectLock(projectPath);
        return projectLock != null && projectLock.isLocked();
    }

    @Override
    public boolean hasProjectLock() {
        Long threadId = Thread.currentThread().getId();
        Collection<ProjectLock> projectLocks = threadProjectMap.get(threadId);
        if (!projectLocks.isEmpty()) {
            for (ProjectLock projectLock : projectLocks) {
                if (projectLock.hasLock()) {
                    return true;
                }
            }
        }
    ...
    public void withProjectLock(final String projectPath, Runnable runnable) {
        ...
        if (!hasProjectLock(projectPath)) {
            withoutWorkerLease(new Runnable() {
                @Override
                public void run() {
                    ProjectLock projectLock = lockProject(projectPath);
                    threadProjectMap.put(threadId, projectLock);
                }
            });
            try {
                runnable.run();
            } finally {
                ProjectLock projectLock = unlockProject(projectPath);
                threadProjectMap.remove(threadId, projectLock);
            }
        } else {
            runnable.run();
        }
    ...
    public void withoutProjectLock(Runnable runnable) {
        if (hasProjectLock()) {
            final Long threadId = Thread.currentThread().getId();
            for (ProjectLock projectLock : threadProjectMap.get(threadId)) {
                projectLock.unlock();
            }
            try {
                runnable.run();
    ...
    private class ProjectLock {
        ...
        ProjectLock(String projectPath) {
